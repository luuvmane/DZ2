...
def test_set_price_negative(sample_product, capsys):
    sample_product.price = -5
    captured = capsys.readouterr()
    assert "Введена некорректная цена" in captured.out

def test_set_price_positive(sample_product):
    sample_product.price = 15.5
    assert sample_product.price == 15.5
